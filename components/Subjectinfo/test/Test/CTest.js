...
const CTest = ({data,setTests,ido,Tests}) => {
    ...
    function SetQuestionTask(e) {
        var new_t = {...Tests};
        new_t.var[0].question = e;
        setTests(new_t);
    }
    function CreateOptionTask(e) {

        var new_t = {...Tests};
        new_t.var[0].options.push({ text:""});
        setTests(new_t);
    }
    useEffect(() => {
        setTests(Tests);
    ...
    function DeleteVariants(value) {
        var new_t = {...Tests};
        new_t.var[0].options.splice(value, 1);
        if(new_t.var[0].answer === value){
            new_t.var[0].answer = -1;
        }else if(new_t.var[0].answer > value){
            new_t.var[0].answer--;
        }
        setTests(new_t);
    }
    ...
    useEffect(()=>{
        document.getElementById("InputCreateTest_0var_0name_" + ido).value = data.var[0].name;
        document.getElementById("InputCreateTest_0var_0question_" + ido).value = data.var[0].question;
    },[Tests]);
    ...
    return (
        <div id={"ctest" + ido}  className={styles.Main_out}>
            ...
            <div className={styles.Main}>
                ...
                {
                    Tests?.var[0].options.map((key, id) => (
                        <>
                        <Variant answer={Tests.var[0].answer === id} idoo={ido} setTests={setTests} DeleteVariants={DeleteVariants} data={key} ido={id}
                                 Tests={Tests} key={id}/>
                            <div className={styles.razd}></div>
                        </>
                    ))
